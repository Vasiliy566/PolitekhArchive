	...
	class ButtonEventListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			if (Task_10.isSubstring(input1.getText(), "цена ")) {
				try {
					JOptionPane.showMessageDialog(null, (Task_10.price(input1.getText())), "Цена",
			...
			if (Task_10.isSubstring(input1.getText(), "индекс ")) {
				try {
					JOptionPane.showMessageDialog(null, (Task_10.index(input1.getText())), "Информация",
							JOptionPane.PLAIN_MESSAGE);
					input1.setText("");
				} catch (HeadlessException | IOException e1) {
					e1.printStackTrace();
			...
			if (Task_10.isSubstring(input1.getText(), "все")) {
				try {
					String str = "";
					for (int i = 0; i < Task_10.priceRange("диапазон 0 100000").size(); i++) {
						str += Task_10.priceRange("диапазон 0 100000").get(i) + "\n";
					}
			...
			if (Task_10.isSubstring(input1.getText(), "диапазон")) {
				try {
					String str = "";
					for (int i = 0; i < Task_10.priceRange(input1.getText()).size(); i++) {
						str += Task_10.priceRange(input1.getText()).get(i) + "\n";
					}
					JOptionPane.showMessageDialog(null, str, "Список товаров", JOptionPane.PLAIN_MESSAGE);
					input1.setText("");
				} catch (HeadlessException | IOException e1) {
